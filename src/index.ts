...
;(function () {
  ...
  var isAlipay = /iPhone|iPad|iPod/i.test(navigator.userAgent)
  ...
  if (isAlipay && isIOS) {
    ...
    if (stringTag) {
      if (!(Blob as any).prototype[stringTag]) {
        ;(Blob as any).prototype[stringTag] = 'Blob'
      }

      if ('File' in global && !(File as any).prototype[stringTag]) {
        ;(File as any).prototype[stringTag] = 'File'
      }
    }
      ...
      *values() {
      ...
      ['_blob']() {
        ...
        this.forEach((value, name) =>
          typeof value == 'string'
            ? chunks.push(
                p + escape(normalizeLinefeeds(name)) + `"\r\n\r\n${normalizeLinefeeds(value)}\r\n`
              )
            : chunks.push(
                p +
